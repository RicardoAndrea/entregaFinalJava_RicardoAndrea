...
const dibujarCarrito = () => {
    ...
    carrito.forEach(producto => {
        ...
        body.innerHTML = `
        <th><img id="fotoProductoCarrito" src="${imagen}" class="card-img-top" style="width:50%; height: 40%"</th>
        <td>${nombre}</td>
        <td>${cantidad}</td>
...
const dibujarFooter = () => {
    ...
    if(carrito.length > 0){
        ...
        <td></td>
        <td>${generarTotales().efectivoTotal}</td>
        `
...
const generarTotales = () => {
    ...
    const efectivoTotal = carrito.reduce((total, {precio}) => total + precio, 0)
    return {
